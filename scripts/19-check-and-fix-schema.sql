-- First, let's check what columns exist in the enrollments table
DO $$
DECLARE
    enrollment_columns TEXT;
BEGIN
    -- Check if enrollments table exists and what columns it has
    SELECT string_agg(column_name, ', ' ORDER BY ordinal_position)
    INTO enrollment_columns
    FROM information_schema.columns 
    WHERE table_name = 'enrollments' AND table_schema = 'public';
    
    RAISE NOTICE 'Enrollments table columns: %', COALESCE(enrollment_columns, 'Table does not exist');
END $$;
...
-- Create submissions table - we'll use the same column name as enrollments
...
DROP POLICY IF EXISTS "Students can manage submissions" ON submissions;
DROP POLICY IF EXISTS "Faculty can view submissions" ON submissions;
DROP POLICY IF EXISTS "Faculty can update submissions" ON submissions;
...
DROP POLICY IF EXISTS "Students can manage their schedules" ON schedules;
...
-- First, let's check if enrollments table exists and what column it uses
DO $$
DECLARE
...
BEGIN
    ...
    ELSIF has_student_id THEN
        ...
        ON CONFLICT DO NOTHING;
    ELSE
        RAISE NOTICE 'Enrollments table not found or has different column structure';
    END IF;
END $$;
...
-- Create a function to get notifications for faculty
CREATE OR REPLACE FUNCTION get_faculty_notifications(faculty_user_id UUID)
RETURNS TABLE (
  id UUID,
  title TEXT,
  message TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    'New Assignment Submission' as title,
    'You have new assignment submissions to review' as message,
    'assignment' as type,
    NOW() - interval '1 hour' as created_at,
    false as read
  WHERE EXISTS (
    SELECT 1 FROM faculty_courses fc WHERE fc.faculty_id = faculty_user_id
  )
  UNION ALL
  SELECT 
    gen_random_uuid() as id,
    'Course Enrollment' as title,
    'New students have enrolled in your courses' as message,
    'enrollment' as type,
